...
### بيانات المتدرب جوه البوت (تلقائي — مفيش إعداد مطلوب)
البريدج بيوصّل بيانات المتدرب للبوت بطريقتين:
1. **حقن سياق تلقائي (الأساسي):** أول رسالة في المحادثة بتتبعت للبوت ومعاها مقدمة
   «معلومات المتدرب (سياق داخلي…)» فيها الاسم/الإيميل/عدد الكورسات/التقدم/أول 3 كورسات
   (بالاسم والتقدم والدرس الجاي). الـ Autonomous Node **بيشوفها مباشرة** في المحادثة
   ويرحّب بالاسم ويتابع الكورسات — من غير أي إعداد جوه Botpress.
   المقدمة بتتبعت **للبوت فقط** (مش بتظهر في Chatwoot ولا للعميل)، وبتتبعت مرة واحدة
   لكل محادثة (وبتتجدد تلقائيًا لو البيانات اتغيرت — `bp_ctx_sig`).
   - لوج البريدج: `CTX → bot` (اتبعتت) أو `CTX empty` (زائر/الـ endpoint مرجّعش بيانات).
2. **حقل `profile`** بتاع الـ Chat user (ملخص ≤450 حرف) — للقراءة البرمجية لو احتجتها
   في Execute Code: `(user as any).tags?.['chat:profile']`.

> لو البوت بيقول «معرفش اسمك»: شوف لوج Railway — لو لاقيت `CTX empty` يبقى المشكلة من
