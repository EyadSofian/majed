...
- زرار مايك في شريط الكتابة (يظهر فقط لو `DEEPGRAM_API_KEY` متضبط في Railway) → يسجّل صوت → `POST /widget/voice` → البريدج يفرّغه عبر **Deepgram Nova-3 (عربي)**.
- العميل يشوف **التسجيل نفسه** (مشغّل صوت) — مش النص المُفرَّغ. الترانسكربت بيروح للبوت فقط، والودجت يعرض «ماجد يفكّر…» ثم رد ماجد.
- التخزين في Chatwoot: **ملف الصوت** يتسجّل كرسالة العميل (incoming) عشان يفضل صوت عند إعادة فتح المحادثة، والترانسكربت يتحطّ كـ **ملاحظة خاصة (private note)** للموظف فقط — العميل مايشوفش النص خالص (لا حيًّا ولا في السجل).
...
## 9.4. تيزرات مساعدة حسب الصفحة (ماجد في كل صفحة) 🆕

**الفكرة:** ماجد يظهر بوب-أب مخصّص حسب الصفحة اللي العميل واقف عليها — يساعده في
السياق ده بالظبط. كله مبني فوق نفس محرّك التيزر (`showOn` + `botMessage`).

**التيزرات المدمجة الجديدة (في `majed-widget.js`):**

| التيزر | الصفحة (`showOn`) | الرسالة |
|--------|-------------------|---------|
...
`_SHOWON` / `_CODE` / `_CODE_LABEL`. وبنفس الشكل: `MAJED_LOGIN_TEASER_*` /
`MAJED_CART_TEASER_*` / `MAJED_PAYMENT_TEASER_*` / `MAJED_COMPANY_TEASER_*`.

**الطبقة التانية (مهمة):** التيزر بيفتح الشات ويبعت رسالة بذرة للبوت — لكن إن ماجد
**يرد صح** على كل سياق لازم تتحط تعليماته في الـ Autonomous Node. النص جاهز في
**`integration/BOTPRESS_PAGE_HELP_PROMPT.md`** — انسخه/ادمجه في Botpress Studio.

> ⚠️ ماجد مايقدرش يبعت كلمة سر على الإيميل (الباسوردات مشفّرة) — بيساعد العميل
> يستخدم خاصية «نسيت كلمة المرور» الرسمية بدل كده. متوثّق في ملف الـ prompt.
...
- **الـ CTA بيتغيّر حسب حالة العميل:** الزائر بيشوف «إنشاء حساب مجاني» (لأن الخصم بيتفعّل
  بالتسجيل)، والمسجّل بيشوف «تصفّح الدورات». الاتنين معاهم زرار «🏷️ اعرف خصمك» اللي بيفتح
  الشات برسالة بتشغّل فلو المستشار.
- **مفيش كود خصم خالص** — كود `engo20` **اتلغى**. الخصم بيتفعّل **بإنشاء الحساب**،
  فزرار «إنشاء حساب مجاني» هو الـ CTA للزوار، و«تصفّح الدورات» للمسجّلين.
  (كود عرض الدورة المجانية `free100` شغّال زي ما هو — ده عرض تاني.)
- **النسبة الدقيقة لكل دورة مش معروفة لماجد** — لو العميل سأل «كم الخصم بالضبط؟»
  ماجد ياخد رقم جواله ويحوّله لـ `salesHandoff` عشان مستشار تعليمي يكلّمه ويقول له
  النسبة (شوف `<exact_discount_flow>` في برومبت ماجد). ده مصدر ليدز، مش مجرد رد.
- لو رجع كود في المستقبل لتيزر معيّن: `MAJED_COURSE_CODE` / `MAJED_SHOP_CODE` /
  `MAJED_CART_TEASER_CODE`.
...
## 9.7. رجوع المحادثة للبوت تلقائيًا لو الموظف ما ردّش 🆕

**الفكرة:** بعد أي `[[HANDOFF]]` (تحويل لموظف/تيم)، لو **مفيش موظف بشري ردّ خلال 3 ساعات**، البريدج يرجّع المحادثة لـ `pending` تلقائيًا فيرجع ماجد يرد، ويبعت رسالة اعتذار للعميل.

**ليه ده شغّال ببساطة؟** إيقاف/تشغيل البوت كله مبني على حالة Chatwoot: `open` = البوت مكتوم، `pending` = البوت شغّال. فالرجوع = مجرد `toggle_status` لـ `pending`.

**التنفيذ (في `integration/bridge/index.js`):**
1. `performHandoff` بينده `armHandoffReturn` اللي بيخزّن توقيت التحويل في `custom_attributes` بتاعة المحادثة (`mjd_handoff_at` / `mjd_handoff_team`) + بيراقبها في الذاكرة.
2. حلقة `sweepHandoffReturns` كل 5 دقايق بتفحص المحادثات اللي عدّى عليها الوقت:
   - لو الحالة مابقتش `open` (الموظف قفلها/علّقها) → بتوقف المراقبة.
   - لو فيه **رد موظف بشري** بعد التحويل (مع تجاهل رسالة تأكيد البوت نفسها عن طريق grace period) → بتوقف المراقبة.
   - غير كده → ترجّع `pending` + رسالة اعتذار.
3. `recoverHandoffWatches` وقت إقلاع البريدج بيعيد بناء المراقبة من Chatwoot، فالـ **redeploy في نص الـ 3 ساعات ما بيضيّعش** المؤقّت.

**التحكم من Railway env vars:**

| Env var | الافتراضي | الوظيفة |
|---------|-----------|---------|
| `HANDOFF_AUTO_RETURN` | `true` | تشغيل/إيقاف الميزة (`false` يوقفها) |
| `HANDOFF_TIMEOUT_MINUTES` | `180` | مدة الانتظار قبل الرجوع للبوت (بالدقايق) |
| `HANDOFF_REPLY_GRACE_SECONDS` | `120` | فترة سماح تتجاهل رسالة تأكيد البوت وقت التحويل |
| `HANDOFF_RETURN_MESSAGE` | رسالة اعتذار جاهزة | الرسالة اللي تتبعت للعميل عند الرجوع للبوت |

> 📌 «رد الموظف» = أي رسالة `outgoing` غير خاصة بعد التحويل بفترة الـ grace. أول ما الموظف يرد، المؤقّت بيتلغي ومش بيرجع للبوت.
